...
public:
    ...
    {
        return Quat ( w_, -x_, -y_, -z_ );    // conjugate
    }
    inline Quat operator- ( void ) const
    {
    ...
    {
        return Quat ( w_ + quat.w_, x_ + quat.x_, y_ + quat.y_, z_ + quat.z_ );
    }
    inline Quat operator- ( const Quat& quat ) const
    {
        return Quat ( w_ - quat.w_, x_ - quat.x_, y_ - quat.y_, z_ - quat.z_ );
    }
    inline Quat operator* ( const double& d ) const
    {
        return Quat ( w_ * d, x_ * d, y_ * d, z_ * d );
    }
    ...
    {
        return x_ * quat.x_ + y_ * quat.y_ + z_ * quat.z_ + w_ * quat.w_;
    }
    inline double angle ( void ) const
    {
        return 2.0 * atan2 ( vectorPart().norm(), w_ );
    }
    ...
    inline Vector3d inverseRotateVector ( const Vector3d& v ) const
    {
        return ( ~*this ).rotateVector ( v );
    }
    inline double magnitudeSquared ( void ) const
    {
    ...
    inline Vector3d vectorPart ( void ) const
    {
    ...
    inline static Quat Identity ( void )
    {
        return Quat ( 1, 0, 0, 0 );
    }
    inline static Quat Zero ( void )
    {
        return Quat ( 0, 0, 0, 0 );
    }
