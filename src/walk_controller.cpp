...
void WalkController::init(void)
{
    ...
    body_clearance_ = UNASSIGNED_VALUE;
...
void WalkController::generateWorkspace(shared_ptr<DebugOutput> debug)
{
  ...
  for (leg_it_ = model_->getLegContainer()->begin(); leg_it_ != model_->getLegContainer()->end(); ++leg_it_)
  {
    ...
    for (int d=0; d<360; d+=1)
    {
      ...
      int iterations = 0;
      if (workspace.find(d) == workspace.end()) //Check if bearing key is in map
      {
          workspace.insert(map<int,double>::value_type(d, UNASSIGNED_VALUE));
      }
      ...
      distance_from_default = Vector3d(leg->getCurrentTipPosition() - leg_stepper->getDefaultTipPosition()).norm();
      workspace[d] = min(current_min, distance_from_default);
      while (iterations > 0)
      {
      ...
      debug->generateWorkspace2(workspace, model_);
    }
  }
  int stop_s=clock();
  for (int d=0; d<360; d+=30)
  {
    ROS_INFO("%f", workspace[d]);
  } 
  ROS_INFO("%f", (stop_s-start_s)/double(CLOCKS_PER_SEC)*1000);
  debug->generateWorkspace2(workspace, model_, true);
