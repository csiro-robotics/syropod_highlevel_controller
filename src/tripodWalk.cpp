...
#define A_PHASE (2*pi)	//Swing Phase	WAVE: 5*pi	TRIPOD: pi	RIPPLE: 2*pi	
#define B_PHASE (pi)	//Stance Phase	WAVE: pi	TRIPOD: pi	RIPPLE: pi
#define PHASE_OFFSET (pi)
#define STANCE_FUNC_ORDER (8*A_PHASE)/B_PHASE

#define SWING_START (A_PHASE/2)
#define SWING_END ((A_PHASE/2)+B_PHASE)

#define LEG_SELECION_PATTERN {0,1,2,0,1,2}		//WAVE: {0,1,2,0,1,2}	TRIPOD: {0,1,2,0,1,2}	RIPPLE: {2,1,0,2,1,0}
...
#define MIN_FOOTPRINT_RADIUS 1e10
...
Vector3d GaitController::LegStepper::getPosition(double liftHeight)
{
  ...
  if (phase > SWING_START && phase < SWING_END)
  {
    ...
    Vector3d pos = cubicBezier(nodes, (phase-A_PHASE/2)/B_PHASE);
    ASSERT(pos.squaredNorm() < MAX_POS);
    return pos;
  }
  else // stance
  {
    double t = 2*phase/A_PHASE;
	if (t > 1.0)
		t -= 2*(A_PHASE+B_PHASE)/A_PHASE;
    Vector3d pos = strideVec * 0.5 * -t;
    pos[2] = liftHeight*0.25 * (pow(t, STANCE_FUNC_ORDER) - 1.0);
    ASSERT(pos.squaredNorm() < MAX_POS);
    return pos;
  }
}
	...
	walkPhase(0)
{
	...
	for (int l = 0; l<3; l++)
	{
		...
		minFootprintRadius = min(MIN_FOOTPRINT_RADIUS, rad*FOOTPRINT_DOWNSCALE);
...
void GaitController::update(Vector2d localNormalisedVelocity, double newCurvature, const Pose *bodyOffset)
{
	...
	if (normalSpeed > 0.0 && !isMoving) // started walking again
	{
		...
		if (walkPhase > A_PHASE + B_PHASE/2)
		{
			walkPhase = A_PHASE+B_PHASE;
		}
		else
		{
			walkPhase = A_PHASE/2;
	...
	double phaseLength = A_PHASE + B_PHASE;
	...
	for (int l = 0; l<3; l++)
	{
		for (int s = 0; s<2; s++)
		{
			...
			legStepper.strideVector = (A_PHASE/(A_PHASE+B_PHASE))*
			...
			if (legStepper.phase == A_PHASE+B_PHASE && phase >= (A_PHASE+B_PHASE)/2 && normalSpeed>0) 
			{
			...
			{
				legStepper.phase = A_PHASE+B_PHASE;
			}
			else
			{
